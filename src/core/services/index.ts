...
export function getServiceMapData(
  ...
  Object.keys(methods).forEach((method) => {
    ...
    if (controllerMap[methodWrapper.tags[0]]) {
      ...
      const { parametersRecord, imports, parameters } = getParameters(
      ...
      typeof response !== 'string' &&
        tag.imports.push(...(response.model || []))
      const request = {
        method: method.toUpperCase(),
        description: methodWrapper.summary,
        url: `${path.replace(/\{(.+)\}/, '${$1}')}`,
        name: methodWrapper.operationId.replace(/\_\d+$/, ''),
        responseType: typeof response !== 'string' ? response.type : response,
        ...parametersRecord,
        parameters,
      }
...
export function getServiceName(name: string): string {
  const stringNew = name
    .split('-')
    .map((stringItem) =>
      stringItem.replace(stringItem[0], stringItem[0].toUpperCase()),
    )
    .join('')
...
export function getResponseType(responses: SwaggerResponses) {
  return responses['200'] && responses['200'].schema
    ? formatTypes(responses['200'].schema)
    : 'any'
}

export const createAlias = (name: string): string | undefined =>
  name.search(VARIABLES_ILLEGAL_REG) > -1
    ? name.replace(VARIABLES_ILLEGAL_REG, '_')
    : undefined
 ...
 *
 */
export function getParameters(parameters: Parameter[]): {
