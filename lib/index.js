...
async function writeMustacheFile(temp, view, src) {
    try {
        ...
        await undefined(path__namespace.join(src, `./${view.name}.ts`), modelData);
    }
    catch (err) {
...
var Stream = _interopDefault$1(require('stream'));
var http = _interopDefault$1(require('http'));
var Url = _interopDefault$1(require('url'));
var https = _interopDefault$1(require('https'));
var zlib = _interopDefault$1(require('zlib'));
...
async function getTemplates() {
    ...
    const temps = await fs__namespace.readdirSync(templatePath);
    const templates = {};
    for (let i = 0; i < temps.length; i++) {
        templates[temps[i].split('.mustache')[0]] = await fs__namespace.readFileSync(path__namespace.join(templatePath, '/' + temps[i]), 'utf-8');
    }
    return templates;
}
async function fetchApiJson(url) {
    try {
        const res = await fetch$1(url).then(res => res.json());
        return res;
    }
    catch (err) {
...
async function main({ url, output, requestPath }) {
    ...
    if (res) {
        const isExists = await undefined(folder);
        if (isExists) {
            await undefined(folder);
        }
        await undefined(folder);
        if (!requestPath) {
...
async function writeExports(templates, folder) {
    const [models, services] = await Promise.all([
        undefined(folder + '/models'),
        undefined(folder + '/services')
    ]);
